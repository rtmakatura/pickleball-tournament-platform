...
export const createMentionNotifications = async (comment, event, members, options = {}) => {
  try {
    ...
    console.log('👤 Comment author:', { authorId: comment.authorId, authorName, authorMember: !!authorMember });
    ...
    console.log(`🎯 Notification targets after filtering:`, {
      totalMentions: mentions.length,
      targetCount: notificationTargets.length,
      allowSelfMentions,
      targets: notificationTargets.map(m => `${m.firstName} ${m.lastName} (${m.id})`)
    });
    
    if (notificationTargets.length === 0) {
      console.log('⚠️ No notification targets after filtering - exiting');
      return;
    }
    ...
    console.log(`💾 Saving ${notifications.length} notifications to Firestore...`);
    const results = await Promise.all(notifications.map(async (notification, index) => {
      try {
        const docRef = await firebaseOps.create('notifications', notification);
        console.log(`✅ Notification ${index + 1} saved with ID:`, docRef);
        return docRef;
      } catch (error) {
        console.error(`❌ Failed to save notification ${index + 1}:`, error);
        throw error;
      }
    }));
    
    console.log(`✅ Created ${results.length} mention notifications successfully`);
  } catch (error) {
    console.error('❌ Error creating mention notifications:', error);
    throw error; // Re-throw so we can see the error in the calling function
  }
};
